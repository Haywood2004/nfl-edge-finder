...
Distribution : Normal(mean, sd) with sd = SCALE_CAL * E|resid| * sqrt(pi/2). SCALE_CAL is fitted on
               the validation season so that the 10/25/75/90 predicted quantiles cover correctly.
P(over line) = 1 - Phi((line + 0.5 - mean) / sd)   (+0.5 continuity; lines are x.5 so no pushes)
...
PARAMS = dict(objective="regression", learning_rate=0.02, num_leaves=15, min_data_in_leaf=40,
...
def _oof_residuals(X, y, seasons, n_iter):
    """Out-of-fold |residuals| by season so the scale model isn't trained on overfit residuals."""
    res = np.zeros(len(y))
    for s in np.unique(seasons):
        tr = seasons != s
        m = _fit_mean(X[tr], y[tr], n_iter=n_iter)
        res[~tr] = np.abs(y[~tr] - m.predict(X[~tr]))
    return res
...
class PassingYardsModel:
    def __init__(self, names, mean_model, scale_model, scale_cal, n_iter):
    ...
    def predict(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        q = {f"q{p}": mean + norm.ppf(p / 100) * sd for p in (10, 25, 50, 75, 90)}
        return pd.DataFrame({"mean": mean, "sd": sd, **q})

    def p_over(self, mean, sd, line):
        return 1 - norm.cdf((line + 0.5 - mean) / sd)
...
def fit(df: pd.DataFrame, names: list[str], train_seasons, valid_season) -> PassingYardsModel:
    ...
    sm = lgb.train(SCALE_PARAMS, lgb.Dataset(tr[names], oof), num_boost_round=max(100, n_iter // 2))
    k = _calibrate_scale(m.predict(va[names]), np.clip(sm.predict(va[names]), 20, None), va.y.values)
    return PassingYardsModel(names, m, sm, k, n_iter)
...
def train(persist: bool = True) -> int:
    ...
    prod.mean_model = _fit_mean(df[names], df.y, n_iter=prod.n_iter)
...
def write_model_md(metrics: dict, names: list[str], run_id: int):
    ...
    lines = [f"# MODEL.md — passing yards ({VERSION})", "",
             ...
             "in games with ≥10 attempts. Distribution = Normal(mean, heteroscedastic sd). "
