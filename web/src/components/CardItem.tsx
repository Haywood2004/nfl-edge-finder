    ...
    <article className={`card p-4 sm:p-5 ${c.published ? "ring-1 ring-up/30" : ""}`}>
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5">
        ...
        <h3 className="text-[15px] font-semibold tracking-tight">
          <Link href={href} className="hover:underline">{isMl ? c.team : c.player_name}</Link>
        </h3>
        ...
        <span className="ml-auto text-[12px]">
          {c.basis === "raw" ? <span className="pill pill-warn">raw model · not a pick</span> : c.published ? <span className="pill pill-up">Flagged</span> : <span className="pill">paper · below bar</span>}
      ...
      <div className="mt-3 flex items-center justify-between text-[12px] text-muted">
        ...
        <Link href={href} className="text-accent hover:underline">Full detail →</Link>
      </div>
    </article>
  );
}
