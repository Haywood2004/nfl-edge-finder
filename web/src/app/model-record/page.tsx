...
export const metadata = { title: "Model paper record" };

export default async function TrackRecord() {
  const [t, br] = await Promise.all([trackRecord(), trackBreakdown()]);
  // one row per pick: the FIRST published version (the price a follower could have taken), not every re-score
  const rows = await sql`
    SELECT DISTINCT ON (c.season, c.week, c.market, c.player_name, c.side)
           c.season, c.week, c.player_name, c.team, c.market, c.side, c.line, c.price_american, c.book, c.edge, c.confidence,
           g.actual, g.result, g.profit_units, g.clv_prob, c.kickoff_utc, c.score
  ...
  return (
    <div className="space-y-8">
      <div>
        <p className="eyebrow">Model paper record</p>
        <h1 className="mt-1 text-2xl font-semibold tracking-tight">Every pick the model flagged</h1>
        <p className="mt-1 max-w-2xl text-sm text-muted">Systematic and unfiltered: one row per pick at the first published price, graded whether or not anyone bet it. Your own bets are on the <a href="/track-record" className="text-accent hover:underline">track record</a>.</p>
      </div>
